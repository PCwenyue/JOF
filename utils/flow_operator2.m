function [A, b, params, iterative] = flow_operator2(this, uv, duv, It, Ix, Iy, weightx, weighty)
...
% THIS SOFTWARE.        
  ...
  % spatial term
  S = this.spatial_filters;%S(1) for grad_x operator; S(2) grad_y operator

  FU = sparse(npixels, npixels);
  FV = sparse(npixels, npixels);
  for i = 1:length(S)%S(1) for d/dx; S(2) for d/dy
      if i == 1
          FMi = spdiags(weightx, 0, npixels, npixels)*make_convn_mat(S{i}, sz, 'valid', 'sameswap');%differential operator
      else
          FMi = spdiags(weighty, 0, npixels, npixels)*make_convn_mat(S{i}, sz, 'valid', 'sameswap');
      end
      Fi  = FMi';

      % Use flow increment to update the nonlinearity
      u_        = FMi*reshape(uv(:, :, 1)+duv(:, :, 1), [npixels 1]);  %du/d*, 925x1    
      v_        = FMi*reshape(uv(:, :, 2)+duv(:, :, 2), [npixels 1]);  %dv/d*, 925x1    

      if isa(this.rho_spatial_u{i}, 'robust_function')          
          pp_su     = deriv_over_x(this.rho_spatial_u{i}, u_);%2nd order derivative 2*10^6 (925x1)
          pp_sv     = deriv_over_x(this.rho_spatial_v{i}, v_);%2nd order derivative 2*10^6 (925x1)          
