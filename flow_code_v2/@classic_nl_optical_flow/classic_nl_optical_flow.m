...
%       this.sor_max_iters   = 1e3; % 调参
      this.sor_max_iters   = 1e4;       % 100 seems sufficient原始
%       this.pcg_iters = 10; % % 调参新增
%       this.pcg_tol = 1e-6; % % 调参新增
      ...
      this.max_iters       = 5; % 原来10次           % number of warping per pyramid level
...
%       this.input_seg = []; % 新增调参
%       this.input_occ = []; % 新增调参
%       this.isScale = true; % 新增调参
      
      this.fullVersion      = false;
%       this.warpingIncrement = 0;% 新增调参
%       this.wmfIters         = 1;% 新增调参
